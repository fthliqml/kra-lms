...
export default function DevelopmentPage({ developments }) {
	...
	const columns = [
		{
			key: "id",
			header: "No.",
			className: "text-center font-semibold md:w-[100px]",
		...
		},
		{
			...
			render: (_, row) => (
				<ActionButton
					...
					onClick={() => {
						setSelectedRow(row);
						setDialogTitle(`${row.year} Development Plan`);
						setOpenDialog(true);
					}}
				/>
			),
	...
	return (
		...
		>
			<div className="w-full flex justify-between items-center mb-5 md:mb-10 flex-col md:flex-row gap-10 md:gap-5">
				<h1 className="text-primary text-4xl font-bold">My Development</h1>
				...
				<div className="flex gap-2 w-full md:w-[350px]">
					<Button
						...
						onClick={() => {
							setSelectedRow(null);
							setDialogTitle("Add Development Plan - Training");
							setOpenDialog(true);
						}}
					>
					...
					<form>
						<DialogHeader>
							...
							<DialogFooter className={"flex flex-row justify-end mt-4"}>
								...
								<Button variant={"primary"} className={"ms-auto"} size={"xl"}>
